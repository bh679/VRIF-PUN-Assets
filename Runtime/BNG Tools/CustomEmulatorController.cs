...
namespace BrennanHatton.VRIF
{

	public class CustomEmulatorController : MonoBehaviour {
		...
		public void CheckHeadControls() {
			...
			// Lock
			if (LockCameraAction == null || LockCameraAction.action.ReadValue<float>() == 1) {
		...
		public InputActionReference rotateHands;
		...
		public Vector2 scrollMax = new Vector2(-180,180);
		float scroll = 0;
		public virtual void UpdateControllerPositions() {
			//Vector2.zero;
			if(rotateHands != null)
				scroll += rotateHands.action.ReadValue<float>()*scrollSpeed;
			
			if(scroll > scrollMax.y)
				scroll = scrollMax.y;
				
			if(scroll < scrollMax.x)
				scroll = scrollMax.x;
			Debug.Log(scroll);
			
			leftControllerTranform.transform.localPosition = LeftControllerPosition;
			leftControllerTranform.transform.localEulerAngles = Vector3.forward * scroll;//.x;

			rightControllerTranform.transform.localPosition = RightControllerPosition;
			rightControllerTranform.transform.localEulerAngles = -Vector3.forward * scroll;//.y;
		}
